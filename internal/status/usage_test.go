...
// parseArgs calls Init on cliFlags, so a case that runs after a failed
...
func resetFlags(t *testing.T) {
	...
	t.Cleanup(func() {
		cliFlags.Init(commandName+" status", flag.ExitOnError)
...
func TestParseArgsHelpGoesToStdout(t *testing.T) {
	for _, arg := range []string{"-h", "--help", "-help"} {
		t.Run(arg, func(t *testing.T) {
			...
			if !strings.HasPrefix(stdout.String(), "Usage: "+commandName+" status") {
...
func TestParseArgsHelpListsEveryFlag(t *testing.T) {
	...
	cliFlags.VisitAll(func(f *flag.Flag) {
...
func TestParseArgsRejectsBadInvocations(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if !strings.Contains(stderr.String(), commandName+" status --help") {
