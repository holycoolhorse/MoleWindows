...
// windowsNoiseInterfacePrefixes match Windows adapter friendly names (lowercased)
// for loopback, tunneling, Hyper-V/WSL virtual switches, and Wi-Fi Direct.
// The macOS "lo" prefix would hide a real "Local Area Connection" adapter.
var windowsNoiseInterfacePrefixes = [...]string{
	"loopback", "isatap", "teredo", "6to4", "vethernet", "bluetooth network connection", "local area connection*",
}
...
func isNoiseInterface(name string) bool {
	return isNoiseInterfaceFor(runtime.GOOS, name)
}

func isNoiseInterfaceFor(goos, name string) bool {
	lower := strings.ToLower(name)
	prefixes := noiseInterfacePrefixes[:]
	if goos == "windows" {
		prefixes = windowsNoiseInterfacePrefixes[:]
	}
