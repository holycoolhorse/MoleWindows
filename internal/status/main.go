// Package status provides the mo status (mole status on Windows) command for real-time system monitoring.
package status
...
// cliFlags is this command's own flag set. Analyze and status share one
// binary on Windows, so neither may register flags on flag.CommandLine.
var cliFlags = flag.NewFlagSet(commandName+" status", flag.ContinueOnError)

var (
	// Command-line flags
	jsonOutput       = cliFlags.Bool("json", false, "output metrics as JSON instead of TUI")
	procCPUThreshold = cliFlags.Float64("proc-cpu-threshold", 100, "alert when a process stays above this CPU percent")
	procCPUWindow    = cliFlags.Duration("proc-cpu-window", 5*time.Minute, "continuous duration a process must exceed the CPU threshold")
	procCPUAlerts    = cliFlags.Bool("proc-cpu-alerts", true, "enable persistent high-CPU process alerts")

	// Watch mode: stream NDJSON (one snapshot per line) from a single warm collector.
	watchMode     = cliFlags.Bool("watch", false, "stream metrics continuously as newline-delimited JSON instead of the one-shot TUI/JSON")
	watchInterval = cliFlags.String("interval", "", "with --watch, collection interval (e.g. 1s, 2s); defaults to 1s")
)
...
const usageText = `Usage: ` + commandName + ` status [OPTIONS]
...
func parseArgs(args []string, stdout, stderr io.Writer) (int, bool) {
	cliFlags.Init(commandName+" status", flag.ContinueOnError)
	...
	cliFlags.SetOutput(io.Discard)
	cliFlags.Usage = func() {}

	err := cliFlags.Parse(args)
	switch {
	...
	case err != nil:
		...
		_, _ = fmt.Fprintln(stderr, "Use '"+commandName+" status --help' for usage information")
		return 1, false
	}
...
func Main() {
	...
	if *watchMode {
		...
		if err != nil {
			...
			fmt.Fprintln(os.Stderr, "Use '"+commandName+" status --help' for usage information")
			os.Exit(1)
		}
