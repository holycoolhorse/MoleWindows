...
func TestIsNoiseInterfaceWindows(t *testing.T) {
	tests := []struct {
		input string
		want  bool
	}{
	...
	for _, tt := range tests {
		if got := isNoiseInterfaceFor("windows", tt.input); got != tt.want {
			t.Errorf("isNoiseInterfaceFor(windows, %q) = %v, want %v", tt.input, got, tt.want)
		}
	}
}
...
func TestIsNoiseInterface(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := isNoiseInterfaceFor("darwin", tt.input)
			if got != tt.want {
				t.Errorf("isNoiseInterfaceFor(darwin, %q) = %v, want %v", tt.input, got, tt.want)
			}
		})
	}
}
