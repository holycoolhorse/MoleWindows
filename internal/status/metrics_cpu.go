...
// cpuPercentFunc is replaceable in tests.
...
func fallbackCPUUtilization(logical int) (float64, []float64, error) {
	...
	if runtime.GOOS == "windows" {
		// Windows has no ps. The machine-wide figure comes from
		// GetSystemTimes, which works where the per-processor query does
		// not; spread it evenly and mark the per-core values as estimated.
		totals, err := cpuPercentFunc(cpuSampleInterval, false)
		if err != nil {
			return 0, nil, err
		}
		if len(totals) == 0 {
			return 0, nil, errors.New("cpu percent returned no data")
		}
		perCore := make([]float64, logical)
		for i := range perCore {
			perCore[i] = totals[0]
		}
		return totals[0], perCore, nil
	}
