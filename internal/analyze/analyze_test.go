...
func TestScanPathConcurrentDedupsHardlinks(t *testing.T) {
	skipUnlessDarwin(t, "NTFS hardlinks are not deduplicated")
	root := t.TempDir()
...
func TestPruneAnalyzerCacheDirIgnoresRemoveFailures(t *testing.T) {
	skipUnlessDarwin(t, "directory permission bits cannot block deletes on Windows")
	if os.Geteuid() == 0 {
...
func TestLiveScanCancellationStopsFoldedDirectoryProbe(t *testing.T) {
	skipUnlessDarwin(t, "stubs du(1), which Windows sizing never runs")
	root := t.TempDir()
...
func TestCacheBypassSkipsHomeLibraryOverviewSnapshot(t *testing.T) {
	...
	library := filepath.Join(home, homeLibraryDirName)
...
func TestIsHandledByMoClean(t *testing.T) {
	skipUnlessDarwin(t, "mo clean paths are macOS paths")
	tests := []struct {
		name string
		path string
		want bool
...
func TestScanPathPermissionError(t *testing.T) {
	skipUnlessDarwin(t, "chmod 000 does not deny reads on Windows")
	root := t.TempDir()
...
func TestSystemOverviewRootsDefaultsToRealSystemPaths(t *testing.T) {
	skipUnlessDarwin(t, "asserts the macOS overview roots")
...
func TestDeleteViewHidesZeroTally(t *testing.T) {
	...
	if !strings.Contains(view, "moving to "+trashLabel) {
...
func TestResolveScanTargetRouting(t *testing.T) {
	cases := []struct {
		...
		{name: "explicit arg is a directory scan", args: []string{"/tmp"}, wantPath: mustAbs(t, "/tmp")},
		{name: "env target is a directory scan", envPath: "/tmp", wantPath: mustAbs(t, "/tmp")},
		{name: "env target wins over args", envPath: "/tmp", args: []string{"/var"}, wantPath: mustAbs(t, "/tmp")},
