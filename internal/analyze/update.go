			...
			if len(removedPaths) > 0 {
				...
				for currentPath != "/" && currentPath != "" {
					...
					if parent == currentPath {
						// A volume root such as C:\ is its own parent.
						break
					}
					currentPath = parent
				}
				...
				for i := range m.history {
					...
					if histPath == m.path || strings.HasPrefix(m.path, histPath+string(filepath.Separator)) {
	...
	case tickMsg:
		...
		if m.scanning || m.deleting || (m.inOverviewMode() && (m.overviewScanning || hasPending)) {
			...
			if m.deleting && m.deleteCount != nil {
				...
				if count > 0 {
					m.status = fmt.Sprintf("Moving to "+trashLabel+"... %s items", formatNumber(count))
				}
			}
			return m, tickCmd()
		}
		...
		if m.showLargeFiles {
			if len(m.largeFiles) > 0 {
				if len(m.largeMultiSelected) > 0 {
					...
					m.status = fmt.Sprintf("Showing %d items in "+fileManagerLabel+"...", count)
				} else {
					selected := m.largeFiles[m.largeSelected]
					go func(path string) {
						_ = safeOpen(path, true)
					}(selected.Path)
					m.status = fmt.Sprintf("Showing %s in "+fileManagerLabel+"...", selected.Name)
				}
			}
		} else if len(m.entries) > 0 {
			if len(m.multiSelected) > 0 {
