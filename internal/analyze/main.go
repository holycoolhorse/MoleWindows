...
// Package analyze implements the analyze disk explorer: an interactive TUI and
// a JSON mode that size directories and move selections to the Trash or the
// Recycle Bin.
package analyze
...
var cliFlags = flag.NewFlagSet(commandName+" analyze", flag.ContinueOnError)

var (
	jsonMode = cliFlags.Bool("json", false, "output analysis as JSON instead of TUI")
)
...
const usageText = `Usage: ` + commandName + ` analyze [OPTIONS] [PATH]
...
func parseArgs(args []string, stdout, stderr io.Writer) (int, bool) {
	cliFlags.Init(commandName+" analyze", flag.ContinueOnError)
	...
	case err != nil:
		...
		_, _ = fmt.Fprintln(stderr, "Use '"+commandName+" analyze --help' for usage information")
		return 1, false
	}
	return 0, true
}
...
func Main() {
	...
	abs, isOverview, err := resolveScanTarget(os.Getenv("MO_ANALYZE_PATH"), cliFlags.Args())
	if err != nil {
...
func newModel(path string, isOverview bool) model {
	...
	currentPath.Store("")
	diskFreeBytes := diskFreeBytesForPath(path)
...
func createOverviewEntriesWithInsights(insightEntries []dirEntry) []dirEntry {
	...
	// Separate Home and its per-user library (~/Library, %USERPROFILE%\AppData)
	// to avoid double counting.
	if home != "" {
		entries = append(entries, dirEntry{Name: "Home", Path: home, IsDir: true, Size: -1})

		userLibrary := filepath.Join(home, homeLibraryDirName)
		if _, err := os.Stat(userLibrary); err == nil {
			...
			entries = append(entries, dirEntry{Name: userLibraryLabel, Path: userLibrary, IsDir: true, Size: -1})
		}
	}
...
func safeOpen(path string, reveal bool) error {
	...
	defer cancel()
	return openPathCommand(ctx, path, reveal).Run()
}

// safePreview opens the file with the platform's default application.
func safePreview(path string) error {
	return safeOpen(path, false)
}
