...
// Init on cliFlags, so a case that runs after a failed parse would
// otherwise inherit the previous run's error handling.
func resetFlags(t *testing.T) {
	t.Helper()
	saved := *jsonMode
	t.Cleanup(func() {
		cliFlags.Init(commandName+" analyze", flag.ExitOnError)
		*jsonMode = saved
	})
}
...
func TestParseArgsHelpGoesToStdout(t *testing.T) {
	for _, arg := range []string{"-h", "--help", "-help"} {
		t.Run(arg, func(t *testing.T) {
			...
			if !strings.HasPrefix(stdout.String(), "Usage: "+commandName+" analyze") {
...
func TestParseArgsUnknownFlagExitsOne(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), commandName+" analyze --help") {
...
func TestParseArgsAcceptsKnownFlagsAndPath(t *testing.T) {
	...
	if got := cliFlags.Args(); len(got) != 1 || got[0] != "/Volumes" {
