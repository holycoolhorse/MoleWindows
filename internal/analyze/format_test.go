	...
	tests := []struct {
	...
	}{
		{
			...
			setup: func() string {
				home := t.TempDir()
				setTestHome(t, home)
				return filepath.Join(home, "Documents", "file.txt")
			},
			check: func(t *testing.T, result string) {
				if !strings.HasPrefix(result, "~"+string(filepath.Separator)) {
					t.Errorf("Expected path to start with ~%c, got %q", filepath.Separator, result)
				}
				if !strings.HasSuffix(result, filepath.Join("Documents", "file.txt")) {
