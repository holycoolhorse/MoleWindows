...
var spotlightQueryRunner = runSpotlightQuery

// errDuUnavailable makes du-based sizing fall through to the Go walk on
// platforms without a trusted du(1).
var errDuUnavailable = errors.New("du sizing is unavailable on this platform")
...
func scanPathConcurrentWithLimiter(ctx context.Context, root string, filesScanned, dirsScanned, bytesScanned *int64, currentPath *atomic.Value, useSpotlight bool, entryLimit int, limiter *scanLimiter, cachePolicy scanCachePolicy, publication *scanPublication) (scanResult, error) {
	...
	isRootDir := isFilesystemRoot(root)
	home := userHomeDir()
...
scanChildren:
	for _, child := range children {
		...
		if child.IsDir() {
			...
			if isHomeDir && child.Name() == homeLibraryDirName {
...
func shouldFoldDirWithPath(name, path string) bool {
	...
	if strings.Contains(path, sep+".npm"+sep) || strings.Contains(path, sep+".tnpm"+sep) {
...
func measureOverviewSize(path string) (int64, error) {
	...
	excludePath := ""
	if home != "" && path == home {
		excludePath = filepath.Join(home, homeLibraryDirName)
	}
...
func getDirectorySizeFromDuWithExcludeAndIgnores(ctx context.Context, path string, excludePath string, ignoreNames []string) (int64, error) {
	...
	runDuSize := func(target string) (int64, error) {
		...
		if !duSizingAvailable {
			// No trusted du on this platform; callers fall back to the Go walk.
			return 0, errDuUnavailable
		}
